...
# Optional PaddleOCR PP-Structure integration
try:
    from paddleocr import PPStructure  # type: ignore
except Exception:
    PPStructure = None

# Optional pytesseract for cell OCR fallback
try:
...
class TableDetector:
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # PP-Structure (optional)
        self.use_ppstructure = bool(self.config.get("use_ppstructure", True)) and (PPStructure is not None)
        self.pp_engine = None
        if self.use_ppstructure:
            try:
                self.pp_engine = PPStructure(show_log=False)
                logger.info("PP-Structure table engine initialized")
            except Exception as e:
                logger.warning(f"PP-Structure unavailable: {e}")
                self.use_ppstructure = False

        # Load detection model (optional)
        self._load_model()

    # Legacy compatibility methods used by EnhancedDocumentProcessor
    ...
    def extract_table_text(self, image_path: str, bbox: List[int]) -> List[List[str]]:
        try:
            import cv2
            img = cv2.imread(image_path)
            if img is None:
            ...
            return self._extract_from_image(table_img)
        except Exception:
            return []
    ...
    def _load_model(self):
        try:
            ...
            if not os.path.exists(model_path):
                logger.debug(f"Model not found at {model_path}, using default detection")
                return
