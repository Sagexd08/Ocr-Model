"""
Dependencies for FastAPI routes (auth, rate limit, validation).
"""
import os
import logging
...
async def get_current_user() -> Optional[dict]:
    """Return current user or None. In tests, return a simple user."""
    if os.getenv("CURIO_TEST_MODE", "0") == "1":
        return {"user_id": "test-user"}
    return None

async def rate_limit() -> None:
    """No-op rate limiter in tests; otherwise best-effort without failing."""
    if os.getenv("CURIO_TEST_MODE", "0") == "1":
        return None
    try:
        # If you have Redis-based rate limiting, it would go here.
        return None
...
async def validate_file(file: UploadFile) -> None:
    """Basic file validation based on content type and size."""
    allowed = {"application/pdf", "image/jpeg", "image/png", "image/tiff", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"}
    if file.content_type not in allowed:
        raise HTTPException(status_code=400, detail="Unsupported file type")
    # Size checks could be added; for tests, accept as-is.
    return None
