...
def render_region_overlays(image: Image.Image, regions: List[Dict[str, Any]], show_labels: bool = True) -> Image.Image:
    """Render region classifier overlays on an image."""
    ...
    try:
        ...
        font = ImageFont.load_default()
    for r in regions:
        bbox = r.get("bbox") or r.get("bbox", [0,0,0,0])
        x1, y1, x2, y2 = [int(v) for v in bbox]
        color = "#007bff"
        draw.rectangle([x1, y1, x2, y2], outline=color, width=2)
        if show_labels:
            label = r.get("type", "region")
